...
@Service
public class MovieCacheRecommenderService {
    @Autowired
    private RedisTemplate<String, Object> redisTemplate;

    @Autowired
    private UserService userService;  // 전체 사용자 ID 가져오는 서비스 (구현 필요)
    @Autowired
    ...
    public static List<String> recommendMovies(int userId)  {
        ...
        if (!userWatchedMovies.lookup(userId).isEmpty()) {
            watchedMovies.addAll((Collection<? extends Integer>) userWatchedMovies.lookup(userId).iterator().next());
        }

        Set<String> watchedDirectors = userWatchedDirectors.lookup(userId).isEmpty()
                ? Collections.emptySet()
                : userWatchedDirectors.lookup(userId).iterator().next();

        Set<String> watchedGenres = userWatchedGenres.lookup(userId).isEmpty()
                ? Collections.emptySet()
                : userWatchedGenres.lookup(userId).iterator().next();
    ...
    // 매일 새벽 3시에 모든 사용자에 대해 추천 계산 후 캐시에 저장
    @Scheduled(cron = "0 0 3 * * *")
    public void trainAndCacheRecommendations() {
        List<Long> allUsers = userService.getAllUserIds(); // 전체 사용자 ID 리스트 획득
        for (Long userId : allUsers) {
            List<String> recommendations = recommendMovies(userId.intValue());
            redisTemplate.opsForValue().set("recommend:" + userId, recommendations, 1, TimeUnit.DAYS);
        }
    }
